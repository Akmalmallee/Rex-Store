...
<div class="max-w-2xl mx-auto space-y-6">
    <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Promo</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Update promotional campaign</p>
    </div>
        ...
        <form action="{{ route('admin.promos.update', $promo) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                    class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-[#C8A951] focus:ring-[#C8A951] @error('description') border-red-500 @enderror">{{ old('description', $promo->description) }}</textarea>
            ...
            <div>
                ...
                @if($promo->image)
                <div class="mb-3">
                    <img src="{{ asset('storage/' . $promo->image) }}" alt="{{ $promo->title }}" class="w-48 h-28 rounded-lg object-cover">
                </div>
                @endif
            ...
            <div>
                ...
                <input type="number" id="discount_percent" name="discount_percent" value="{{ old('discount_percent', $promo->discount_percent) }}" required min="0" max="100"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="datetime-local" id="start_at" name="start_at" value="{{ old('start_at', $promo->start_at ? $promo->start_at->format('Y-m-d\TH:i') : '') }}" required
                ...
                <div>
                    ...
                    <input type="datetime-local" id="end_at" name="end_at" value="{{ old('end_at', $promo->end_at ? $promo->end_at->format('Y-m-d\TH:i') : '') }}" required
